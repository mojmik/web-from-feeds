	...
	function importCSVprocAjax() {
		...
		if ($do == "createCats") {
			if ($this->isCJ) {
				...
				echo json_encode(["result" => "cats not needed"]) . PHP_EOL;
			}
			wp_die();
		}
	...
	function editCptHtml() {
		...
		<div class='caf-editFieldRow'>
			<form id="mAutaEdit<?= $this->getCustomPostType(); ?>" method='post' class='caf-editFieldRow caf-noborder editCPT'>
				<div>
					<div><label>singular name</label></div><input type='text' name='singular' value='<?= $this->singular ?>' />
				</div>
				<div>
					<div><label>plural name</label></div><input type='text' name='plural' value='<?= $this->plural ?>' />
				</div>
				<div>
					<div><label>special type</label></div><select name='specialType'><?= Mutils::optionsStringFromArray($specialTypes, $this->specialType) ?></select>
				</div>
				<div>
					<div><label>dedicated table? *</label></div><input name='tableType' type='checkbox' <?= ($this->tableType == "dedicated" ? "value='1' checked='checked'" : "") ?> />
				</div>
